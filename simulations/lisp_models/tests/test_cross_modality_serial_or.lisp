...
(setf model-path "../simulations/lisp_models/cross_modality_serial_or/cross_modality_serial_or_model.lisp")
(setf task-path "../simulations/lisp_models/common_task_functions.lisp")
(setf model-name "cross-modality serial or")
...
(print (format nil "running ~A" model-name))
(setf true-module-times1 "[[\"VISUAL\",null],[\"PRODUCTION\",[[0.0,0.05],[0.335,0.385]]],[\"MANUAL\",[[0.385,0.685]]],[\"AURAL\",[[0.05,0.335]]]]")
(setf rt1 (run-trial 'present-stimuli-cross-modality "L" 2 parms :seed 70 :track-history t))
(setf module-data1 (process-history-data "module-demand-times"))
; test 1
(if (string= module-data1 true-module-times1) 
...
; test 9
(if (string= module-data3 true-module-times3) 
    (let  () (print-test-result model-name 9 t)
    (incf num-tests-passed))
    (let  () (print-test-result model-name 9 nil)
    (incf num-tests-failed)))
; test 10
(setf true-rt3 '(1 NIL NIL "f" 0.36))
(if (equal true-rt3 rt3) 
    (let  () (print-test-result model-name 10 t)
    (incf num-tests-passed))
    (let  () (print-test-result model-name 10 nil)
    (incf num-tests-failed)))
